...
class particle_filter_cascade:
    ...
    OBSERVATION_LAMBDA_B = "B16"  # beat observation lambda
    OBSERVATION_LAMBDA_D = "B16"  # downbeat observation lambda
    fps = 100
    T = 1 / fps
    MIN_BEAT_PER_BAR = 3
    MAX_BEAT_PER_BAR = 4
    OFFSET = 0 # The point of time after which the inference model starts to work. Can be zero!
    IG_THRESHOLD = 0.15  # Information Gate threshold
    ...
    def __init__(self, beats_per_bar=[], particle_size=PARTICLE_SIZE, down_particle_size=DOWN_PARTICLE_SIZE,
        ...
        self.mode = mode
        self.num_tempi = max_beats_per_bar - min_beats_per_bar + 1
    ...
    def process(self, activations):
        """
        Running Particle filtering over the given activation function to infer beats.

        Parameters
        ----------
        activations : numpy array, shape (num_frames, 1)
            Activation function with probabilities corresponding to beats
            in the first and only column.

        Returns
        -------
        beats : numpy array, shape (num_beats, 1)
            Detected beat positions [seconds] and beat numbers.
        ...
        for i in range(len(activations)):  # loop through the provided frame/s to infer beats
            self.counter += 1
            if 'activations' in self.plot and (self.mode == 'stream' or self.mode == 'realtime'):  # Plotting the activations
                self.activations_plot()
            gathering = int(np.median(self.particles))  # calculating beat particles clutter
            matching_interval_beats = self.beat[self.st.state_intervals[self.beat] == self.st.state_intervals[gathering]]
            close_to_beat = (gathering - matching_interval_beats) < (int(.07 / self.T)) + 1
            enough_time_passed = (self.offset + self.counter * self.T) - self.path[-1][0] > .4 * self.T * self.st.state_intervals[gathering]

            if close_to_beat.any() and enough_time_passed:
                # Ensure time values are formatted to 6 decimal places
                new_beat_time = float(f"{self.offset + self.counter * self.T:.6f}")
                self.path = np.append(self.path, [[new_beat_time, 1]], axis=0)
            ...
            if activations[i] > 0.1:  # resampling is done only when there is a meaningful activation
                ...
                self.particles = universal_resample(self.particles, obs[self.particles])
                if activations[i] > 0.8:
    ...
    def reset(self):
        """Reset the particle filter, set all previous values to 0"""
        self.counter = -1
        self.path = np.zeros((1, 2), dtype=float)
        self.particles = np.sort(np.random.choice(np.arange(0, self.st.num_states - 1), self.particle_size, replace=True))
        self.down_particles = np.sort(np.random.choice(np.arange(0, self.st2.num_states - 1), self.down_particle_size, replace=True))
...
# Define the beat_densities and universal_resample functions here as required
